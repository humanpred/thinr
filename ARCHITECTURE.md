...
dispatching API. Drop-in replacement for `EBImage::thinImage()`.
Designed to be small, focused, CRAN-quality, and
...
2.  **API stability.**
    ...
    are the public surface. Any signature change is a major version
    bump.

3.  **Drop-in compatibility.** `thinImage(x)` must return the same shape
    and storage mode as `EBImage::thinImage(x)` for the same input. The
    default algorithm is therefore locked to Zhang-Suen.

4.  **Stubs error, don’t silently fall back.** Lee and K3M throw with a
    message that names the planned version. Silent fall-backs to
    Zhang-Suen would hide the limitation.

5.  **2-D for now.** Higher-dimensional arrays are explicitly rejected.
