...
| Function               | Reference |
|------------------------|-----------|
| `medial_axis()`        | Blum, H. (1967). A transformation for extracting new descriptors of shape. In *Models for the Perception of Speech and Visual Form* (pp. 362–380). MIT Press. Implementation finds ridge points of the squared Euclidean distance transform; returns the binary skeleton, optionally with per-pixel distance. |
| `distance_transform()` | Felzenszwalb, P. F. & Huttenlocher, D. P. (2012). Distance transforms of sampled functions. *Theory of Computing*, 8(19), 415–428. [doi:10.4086/toc.2012.v008a019](https://doi.org/10.4086/toc.2012.v008a019). Linear-time separable squared Euclidean transform; plus the Rosenfeld & Pfaltz (1968) two-pass forward + backward sweep for `metric = "manhattan"` and `"chessboard"`. |
