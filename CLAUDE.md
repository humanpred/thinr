...
- **Acceptance:** `R CMD check --as-cran` locally reports 0 errors, 0 warnings, NOTEs acceptable for the Ubuntu system compilation-flags note and (once published) CRAN's "package version already exists" incoming-feasibility check on a repeat run of the same version; the GitHub Actions `R-CMD-check.yaml` gate (which does not hit the same incoming-feasibility path) is the authoritative signal and is green on `main`.
